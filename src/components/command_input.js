...
function intent(DOM, game$) {
  ...
  const enterPressed$ = keydown$.filter(({keyCode}) => keyCode === KEYCODE_ENTER);
  const inputStarted$ = input$.compose(throttle(500));
  const inputEnded$ = input$.compose(debounce(500));

  const action$ = xs.merge(
      game$
        .map(val => ({ type: 'GAME_CHANGED', payload: val })),
...
function model(action$, initialState) {
  const inputReducer$ = action$.filter(a => a.type === 'INPUT_CHANGED')
    .map(action => state => ({ ...state, inputValue: action.payload }));

  const inputEndedReducer$ = action$.filter(a => a.type === 'INPUT_ENDED')
    .map(action => state => ({
      ...state,
      feedback: getInputFeedback(state.game, action.payload),
    }));

  const inputStartedReducer$ = action$.filter(a => a.type === 'INPUT_STARTED')
    .map(action => state => ({ ...state, feedback: NO_FEEDBACK }));

  const gameReducer$ = action$.filter(a => a.type === 'GAME_CHANGED')
    .map(action => state => ({ ...state, game: action.payload }));

  const commandSentReducer$ = action$.filter(a => a.type === 'SEND_MOVE_COMMAND')
    .map(action => state => ({ ...state, inputValue: '', feedback: NO_FEEDBACK }));
  ...
  const command$ = action$.filter(a => a.type === 'SEND_MOVE_COMMAND')
  ...
  return state$.map(state => {
    ...
    return <div class={{
        ...
        <input
          ...
          disabled={!ourTurn} />
        <div class={{ [styles.feedback]: true }}>
          {state.feedback.text}
        </div>
      </div>
  });
